...
class ProxyServer:
    ...
    async def _do_direct_tunnel(self, host: str, port: int,
        ...
        Target selection (when ``connect_ip`` not provided):
          - Port 443: route through the configured ``google_ip`` Google
            Front-End. SNI decides which service answers, and this
            sidesteps DNS poisoning on censored networks (the whole
            reason this proxy exists).
          - Other ports (e.g. mtalk XMPP on 5228): connect to the real
            hostname — the GFE IP is HTTPS-only, so a port 5228 tunnel
            there always fails.
        """
        if connect_ip is not None:
            target = connect_ip
        elif port == 443:
            target = self.fronter.connect_host
        else:
            target = host
        try:
            ...
            log.error("Direct tunnel connect failed (%s → %s:%d): %s",
                      host, target, port, e)
            return
